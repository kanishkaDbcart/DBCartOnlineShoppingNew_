...
jQueryAjaxPost = (form, controller, action) => {
    try {
        $.ajax({
            type: 'POST',
            url: form.action,
            ...
            success: function (res) {
                if (res.isValid) {
                    ...
                    //$.notify('Failed to Submit', { globalPosition: 'top center', className: 'success' });
                }
            },
            error: function (err) {
                console.log(err)
...
showInPopupWithId = (url, title, id) => {
    $.ajax({
        type: 'GET',
        url: url,
        data: { id: id },
